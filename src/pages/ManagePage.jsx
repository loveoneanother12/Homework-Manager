...
function ClassForm({ onSave, onCancel, saving }) {
  const [form, setForm] = useState({ class_name: '', days_of_week: '', instructor: '' });

  return (
    <form onSubmit={e => { e.preventDefault(); onSave(form); }}
      className="bg-white border border-indigo-100 rounded-xl p-5 mb-4 shadow-sm space-y-4">
      <h3 className="font-semibold text-gray-800">새 반 추가</h3>
      <div className="grid grid-cols-2 gap-4">
        <div>
          ...
          <input
            ...
            placeholder="예: 중2-A반" autoFocus required />
        </div>
        <div>
      ...
      <div className="flex gap-2">
        <button type="submit" disabled={saving || !form.class_name.trim()}
          className="px-5 py-2 bg-indigo-600 text-white rounded-lg text-sm font-medium hover:bg-indigo-700 disabled:opacity-50 transition-colors">
          {saving ? '저장 중…' : '추가'}
        </button>
        <button type="button" onClick={onCancel}
...
export default function ManagePage() {
  ...
  async function handleAddClass(form) {
    setClassSaving(true);
    try {
      await addClass(form);
  ...
  function deleteModalProps() {
    const { item, recordCount } = deleteTarget;
      ...
      <section>
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex items-center gap-2">
            ...
            />
            <button
              onClick={() => { setShowAddClassForm(true); window.scrollTo({ top: 0, behavior: 'smooth' }); }}
            ...
            {classes.filter(c => !classSearch.trim() || c.class_name.includes(classSearch.trim())).map((cls, idx) => {
              ...
              return (
                <div key={cls.id} className={idx > 0 ? 'border-t border-gray-200' : ''}>
                  <div
                    className="flex items-center px-5 py-4 gap-4 cursor-pointer bg-white hover:bg-gray-50 transition-colors"
